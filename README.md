...
- `适用岗位` 填 `通用` 表示所有岗位必看。
- 解析器支持字段内含逗号、换行、`""` 转义及 UTF-8 BOM（Excel 友好）；**格式错误即整文件拒绝**（HTTP 400 并附行号）：引号未闭合、字段中非法引号、数据行列数与表头不一致，都不会被静默当作有效数据。

**用户**：`用户名,密码,姓名,岗位,角色`。用户名重复则更新（密码列留空表示不改密码）；新用户必须带初始密码。

- `角色` 仅接受白名单精确值：`admin`/`管理员` → 管理员，`volunteer`/`志愿者`/留空 → 志愿者；其他任何取值（如 `notadmin`）该行拒绝导入，**不会**被猜测为管理员。
...
```bash
npm test          # test/e2e.sh：在临时数据库上启动服务并跑 22 项端到端断言
```

覆盖：鉴权拦截、岗位匹配（通用+专项）、跨岗位隔离、CSV 新增/更新/错误行/转义、畸形 CSV 拒绝（未闭合引号/列数不齐）、角色白名单（`notadmin` 不得提权）、反馈幂等与完成不可逆、岗位统计、越权访问、会话密钥落盘位置。
